...
const ProtectedRoute = ({ children, adminOnly = false }: { children: React.ReactNode, adminOnly?: boolean }) => {
  ...
  const isHardcodedAdmin = user?.email === 'kubratekbiyik@gmail.com';
  const shouldWaitProfile = adminOnly && !isHardcodedAdmin && profileLoading;

  if (loading || shouldWaitProfile) {
    return (
...
export default function App() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-bauhaus-bg flex items-center justify-center relative overflow-hidden">
        <div className="absolute inset-0 bauhaus-grid opacity-50" />
        <div className="relative z-10 flex flex-col items-center gap-8">
          <div className="w-24 h-24 border-8 border-bauhaus-ink border-t-bauhaus-red rounded-full animate-spin shadow-[8px_8px_0px_0px_rgba(20,20,20,1)]" />
          <div className="font-display font-black text-2xl tracking-tighter uppercase animate-pulse">
            YÜKLENİYOR...
          </div>
        </div>
      </div>
